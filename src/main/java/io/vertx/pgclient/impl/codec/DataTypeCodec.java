...
class DataTypeCodec {
    ...
    private static final byte[] JSON_NULL = new byte[] {'n', 'u', 'l', 'l'};
    ...
    private static void binaryEncodeJSON(Object value, ByteBuf buff) {
        if (value == Tuple.JSON_NULL) {
            buff.writeBytes(JSON_NULL);
        } else if (value instanceof Buffer) {
            buff.writeBytes(((BufferImpl) value).byteBuf());
        } else {
    ...
    private static Object binaryDecodeJSONB(int index, int len, ByteBuf buff) {
        ...
        return binaryDecodeBYTEA(index + 1, len - 1, buff);
    }
